 ...
 */
    ...
    const updateBatteries = () => {
        fetch('/api/robots')
            .then(res => res.json())
            .then(resData => {
                if (resData.success && resData.data.robots) {
                    resData.data.robots.forEach(robot => {
                        // Actualizar sidebars si existen
                        const sbBat = document.getElementById('sidebar-bat-' + robot.RobotID);
                        if (sbBat) {
                            sbBat.innerHTML = `<i class="ph ph-battery-medium"></i> ${robot.Bateria}%`;
                            sbBat.style.color = "inherit"; 
                        }
                        // Actualizar panel principal para el robot activo (2)
                        if (robot.RobotID === 2) {
    ...
    updateBatteries();
    setInterval(updateBatteries, 1000);
};
...
async function updateHistoryUI() {
    try {
        ...
        // Filtrar específicamente para Roberto 2 ya que está apagado
        fullHistoryData = allData.filter(log => log.Robot === 'Roberto 2');
